...
from clicking.image_processor.anthropic_clicking import AnthropicClicking
import nest_asyncio
...
anthropic_clicking = AnthropicClicking(client, config=config)
#%%
...
pipeline_steps = [
    ...
    PipelineStep(
        ...
        function=anthropic_clicking.get_results,
        mode_keys=["pointing_mode"]
    ),
]
...
def plot_image_with_point(image, clickpoint, linewidth: int = 2):
    ...
    plt.plot([0, width], [clickpoint.y, clickpoint.y], color='black', linewidth=linewidth, linestyle='dotted')
    plt.plot([clickpoint.x, clickpoint.x], [0, height], color='black', linewidth=linewidth, linestyle='dotted')
    plt.plot(clickpoint.x, clickpoint.y, marker='o', color='red', markersize=10)
    print(f"Image size: {image.size}")
    print(f"Clickpoint: {clickpoint}")
    
    plt.show()

for clicking_image in result.images:
    for obj in clicking_image.predicted_objects:
        if obj.clickpoint is None:
