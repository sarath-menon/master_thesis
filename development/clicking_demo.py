...
DETECTION_ENDPOINT = SERVER_URL + "/localization"
...
results = get_model_prediction(image, text_input, task_prompt, url=DETECTION_ENDPOINT)
...
index = 4
SERVER_URL = "http://localhost:8082/"
SEGMENTATION_ENDPOINT = SERVER_URL + "/segmentation"

def get_segmentation_prediction(image, input_boxes, url):
    # Convert PIL image to base64
    buffered = io.BytesIO()
    image.save(buffered, format="PNG")
    img_str = base64.b64encode(buffered.getvalue()).decode("utf-8")
...
to_pil = transforms.ToPILImage()
image = to_pil(image.mul(255).byte())
input_boxes = [[75, 275, 1725, 850], [425, 600, 700, 875]]

results = get_segmentation_prediction(image, input_boxes, url=SEGMENTATION_ENDPOINT)
print(results)
